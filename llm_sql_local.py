...
class LocalLLM:
    ...
    def template_sql(self, intent, question):
        ...
        def top_clause():
            return f"TOP {n} " if n else ""

        if intent == "safe_mode":
            return "SELECT * FROM orders;"

        templates = {
            "list_customers": f"SELECT {top_clause()}* FROM customers;",
            "list_products": f"SELECT {top_clause()}* FROM products;",
            "list_orders": f"SELECT {top_clause()}* FROM orders;",
...
"""
    ...
    # ------------- MAIN ----------------
    def generate_sql(self, schema_text, question: str, result_limit=None):
