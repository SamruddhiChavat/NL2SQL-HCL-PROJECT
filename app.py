...
if user_q:
    ...
    try:
        ...
        if norm_q in {"show all products", "list all products", "get all products"}:
            ...
            direct_sql = "SELECT * FROM customers;"

        elif norm_q in {"show all sellers", "list all sellers", "get all sellers", "show sellers", "list sellers"}:
            direct_sql = "SELECT * FROM sellers;"
...
"""
        ...
        if direct_sql is None and "sellers from" in norm_q:
            try:
                city = user_q.lower().split("from")[1].strip()
                direct_sql = f"""
SELECT *
FROM sellers
WHERE LOWER(seller_city) = LOWER('{city}');
"""
            except:
                pass
...
"""
        ...
        if direct_sql is not None:
            ...
            sql = llm.generate_sql(SCHEMA_TEXT, user_q)
