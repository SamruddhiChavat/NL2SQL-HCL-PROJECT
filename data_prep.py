...
def load_and_clean(csv_path):
    ...
    # optional: cast IDs (if numeric)
    # df['defect_id'] = pd.to_numeric(df['defect_id'], errors='coerce')

    # normalize text columns: strip + replace multiple spaces
    ...
    # Optionally drop rows missing essential fields
    df = df.dropna(subset=['defect_id']) if 'defect_id' in df.columns else df

    # Fill or mark missing severity
